...
class UNO(nn.Module):
    def __init__(self, 
        ...
        """
        self.s = s
        self.time = TimestepEmbedding(embed_dim, 2*self.s, self.s**2)
        ...
        self.fc0 = nn.Linear(self.in_d_co_domain+2+1, self.d_co_domain) # input channel is 3: (a(x, y), x, y)
    ...
    def forward(self, x, sigma):
        ...
        time_embed = self.time(sigma.view(-1,1)).\
            view(-1, self.s, self.s, 1)
